...
namespace Library
{
    public partial class StatisticsBadReader : Form
    {
        public StatisticsBadReader()
        {
            InitializeComponent();
        }
        ...
        private void StatisticsBadReader_Load_1(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                else
                {
                    ...
                    if (comboBox1.Text.Trim() != "")
                        sql = String.Format("select TOP {0} a.rid as 身份证号,Reader.rname as 姓名,a.b as 欠款 from Reader, ReaderCategory,"
                            + "(select rid,sum(money) as b from booklendlist  where booklendlist.LendTime>= '{1}' and  booklendlist.LendTime<= '{2}'    group by rid ) as a "
                            + "where ReaderCategory.Rcategoryname='{3}' and ReaderCategory.Rcategoryid=reader.Rcategoryid  and a.Rid=reader.Rid and a.b>0 "
                            + "order by a.b desc", m, date1, date2, category);
                    else
                        sql = String.Format("select TOP {0} a.rid as 身份证号,Reader.rname as 姓名,a.b as 欠款 from Reader, ReaderCategory,"
                        + "(select rid,sum(money) as b from booklendlist  where booklendlist.LendTime>= '{1}' and  booklendlist.LendTime<= '{2}'    group by rid ) as a "
                        + "where ReaderCategory.Rcategoryid=reader.Rcategoryid  and a.Rid=reader.Rid and a.b>0 "
